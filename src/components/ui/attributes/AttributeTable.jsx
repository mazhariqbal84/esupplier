...
import {
  ...
  setCurrentPage,
  setGlobalSearch,
} from "@/pages/attribute/store/store";
import {
  useTable,
  useRowSelect,
  useSortBy,
...
import GlobalFilter from "@/pages/attribute/attributes/GlobalFilter";
...
const AttributeTable = ({
  ...
  const [totalPages, setTotalPages] = useState(
    attributeData.total / itemsPerPage
  );
  ...
  useEffect(() => {
    ...
    setTotalPages(attributeData.total / itemsPerPage);
  }, [itemsPerPage]);
  ...
  useEffect(() => {
    dispatch(setGlobalSearch(globalFilterInput));
    if (attributeData.total > itemsPerPage) {
      setTotalPages(attributeData.total / itemsPerPage);
    } else {
      setTotalPages(1);
    }
  ...
  const deleteAllAttribute = (event) => {
  ...
  useEffect(() => {
    ...
    setSelectedAttributes(selectedId);
  }, [selectedFlatRows]);
      ...
      <div className="flex md:space-y-0 flex-wrap space-y-5 justify-between mt-10 items-center">
        <div className=" flex items-center space-x-3 rtl:space-x-reverse">
          ...
          <span className="text-sm font-medium text-slate-600 dark:text-slate-300">
            Page{" "}
            <span>
              {currentPage} of {attributeData?.last_page}
            </span>
            <span>
              {Object.keys(selectedRowIds).length > 0 && (
                <select
                  ...
                  onChange={(e) => deleteAllAttribute(e)}
                >
