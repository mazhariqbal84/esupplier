...
import {
  ...
  setCurrentPage,
  setGlobalSearch,
} from "@/pages/product/store/store";
import {
  useTable,
  useRowSelect,
  useSortBy,
...
import GlobalFilter from "@/pages/product/products/GlobalFilter";
...
const ProductTable = ({
  ...
  const [totalPages, setTotalPages] = useState(
    productData.total / itemsPerPage
  );
  ...
  useEffect(() => {
    ...
    setTotalPages(productData.total / itemsPerPage);
  }, [itemsPerPage]);
  ...
  useEffect(() => {
    dispatch(setGlobalSearch(globalFilterInput));
    if (productData.total > itemsPerPage) {
      setTotalPages(productData.total / itemsPerPage);
    } else {
      setTotalPages(1);
    }
      ...
      <div className="flex md:space-y-0 flex-wrap space-y-5 justify-between mt-10 items-center">
        <div className=" flex items-center space-x-3 rtl:space-x-reverse">
          ...
          <span className="text-sm font-medium text-slate-600 dark:text-slate-300">
            Page{" "}
            <span>
              {currentPage} of {productData?.last_page}
            </span>
            <span>
