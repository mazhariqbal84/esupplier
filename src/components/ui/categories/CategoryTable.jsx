...
import {
  ...
  setCurrentPage,
  setGlobalSearch,
} from "@/pages/category/store/store";
import {
  useTable,
  useRowSelect,
  useSortBy,
...
import GlobalFilter from "@/pages/category/categories/GlobalFilter";
...
const CategoryTable = ({
  ...
  const [totalPages, setTotalPages] = useState(
    categoryData.total / itemsPerPage
  );
  ...
  useEffect(() => {
    ...
    setTotalPages(categoryData.total / itemsPerPage);
  }, [itemsPerPage]);
  ...
  useEffect(() => {
    dispatch(setGlobalSearch(globalFilterInput));
    if (categoryData.total > itemsPerPage) {
      setTotalPages(categoryData.total / itemsPerPage);
    } else {
      setTotalPages(1);
    }
      ...
      <div className="flex md:space-y-0 flex-wrap space-y-5 justify-between mt-10 items-center">
        <div className=" flex items-center space-x-3 rtl:space-x-reverse">
          ...
          <span className="text-sm font-medium text-slate-600 dark:text-slate-300">
            Page{" "}
            <span>
              {currentPage} of {categoryData?.last_page}
            </span>
            <span>
              {Object.keys(selectedRowIds).length > 0 && (
                <select
                  ...
                  onChange={(e) => deleteAllCategory(e)}
                >
